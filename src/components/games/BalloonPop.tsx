...
export function BalloonPop({ questions, onComplete }: BalloonPopProps) {
  ...
  const colors = ['from-red-400 to-red-600', 'from-blue-400 to-blue-600', 'from-green-400 to-green-600', 'from-yellow-400 to-yellow-600', 'from-purple-400 to-purple-600', 'from-pink-400 to-pink-600'];

  useEffect(() => {
    if (!question) return;

    // Create balloons for current question
    const newBalloons: Balloon[] = question.options?.map((option, i) => ({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-400 via-blue-500 to-blue-700 overflow-hidden relative">
      {/* Header */}
      <div className="relative z-20 text-center pt-8 pb-4">
        <h2 className="text-white mb-4">Balloon Pop</h2>
        <div className="bg-white rounded-2xl p-6 max-w-md mx-auto mb-4">
          <h3 className="mb-2">Pop the balloon with:</h3>
          ...
          {balloons.map((balloon) => (
            ...
            >
              {poppedBalloon === balloon.id ? (
                ...
                >
                  {/* Balloon */}
                  <div className={`w-24 h-28 bg-gradient-to-br ${balloon.color} rounded-full relative shadow-2xl`}>
                    {/* Shine */}
                    ...
                    {/* Balloon knot */}
                    <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-8 border-transparent border-t-current" 
                      style={{ color: balloon.color.includes('red') ? '#dc2626' : balloon.color.includes('blue') ? '#2563eb' : balloon.color.includes('green') ? '#16a34a' : balloon.color.includes('yellow') ? '#ca8a04' : balloon.color.includes('purple') ? '#9333ea' : '#ec4899' }}
                    />
                  </div>
