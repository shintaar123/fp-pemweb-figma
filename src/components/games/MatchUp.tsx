...
export function MatchUp({ questions, onComplete }: MatchUpProps) {
  const [items, setItems] = useState<MatchItem[]>(() => {
    ...
    const questions_part = matchItems.filter(item => item.type === 'question');
  ...
  const [selectedQuestion, setSelectedQuestion] = useState<MatchItem | null>(null);
  ...
  const handleItemClick = (item: MatchItem) => {
    ...
    if (item.type === 'question') {
      ...
      if (selectedQuestion.value === item.value) {
        ...
        const allMatched = items.every(i => 
          i.id === selectedQuestion.id || i.id === item.id || i.matched
        );
        if (allMatched) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-500 via-teal-500 to-cyan-500 p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h2 className="text-white mb-2">Match Up</h2>
          <p className="text-white/90">Click a question, then click its matching answer</p>
          ...
          <div className="space-y-3">
            {answerItems.map((item) => (
              <motion.button
                ...
                whileHover={!item.matched && selectedQuestion ? { scale: 1.05 } : {}}
                whileTap={!item.matched && selectedQuestion ? { scale: 0.95 } : {}}
                layout
              >
